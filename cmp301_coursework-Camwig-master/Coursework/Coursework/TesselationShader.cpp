...
TesselationShader::TesselationShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"Tesselation_vs.cso", L"Tesselation_hs.cso", L"Tesselation_ds.cso", L"texture_ps.cso");
}
...
void TesselationShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	renderer->CreateBuffer(&matrixBufferDesc, NULL, &tessellationBuffer);
...
void TesselationShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 Outside, XMFLOAT2 Inside, XMFLOAT3 camPos)
{
	HRESULT result;
	...
	//Send tesselation buffer and send it to the hull shader
	...
	tessPtr->Outside_Tesselation = Outside;
	tessPtr->Inside_Tesselation = Inside;
	tessPtr->cam_pos = camPos;
	tessPtr->padding2 = 0.0f;
