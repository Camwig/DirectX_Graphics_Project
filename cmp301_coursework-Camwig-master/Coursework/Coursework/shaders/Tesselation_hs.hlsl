...
ConstantOutputType PatchConstantFunction(InputPatch<InputType, 4> inputPatch, uint patchId : SV_PrimitiveID)
{
    ...
    float3 center = 0.25f * (inputPatch[0].position + inputPatch[1].position + inputPatch[2].position + inputPatch[3].position);
    float3 centerWorld = mul(float4(center, 1.0f), worldMatrix).xyz;
    float d = distance(centerWorld, cam_pos.xyz);
    // Tessellate the patch based on distance from the eye such that

    //Near enough to start the tesselation
    const float d0 = 10.0f;
    //Far enough away to not tesselate anymore
