...
float3 CalcNormal(float2 uv, Texture2D texture0, SamplerState sampler0 , float offset)
{
    ...
    //will check this to offset any issues with the pixels/vertices lining up with the texture
    ...
    //gets the height of the texture to the north of the current vertex/pixel moore neighbourhood
...
float3 CalcNormal2(float2 uv, Texture2D texture0, Texture2D texture1, SamplerState sampler0, float dt,float offset)
{
    //current dimension of the texture since the current texture is a square the dimensions are that of a equal value
    float texture_dimensions = 256.0f;
    //will check this to offset any issues with the pixels/vertices lining up with the texture
    ...
    float North_height2 = getHeight(float2(uv.x, uv.y + uv_offset), texture1, sampler0, offset);
    //does this for the other three cardinal directions
    float South_height2 = getHeight(float2(uv.x, uv.y - uv_offset), texture1, sampler0, offset);
    float East_height2 = getHeight(float2(uv.x + uv_offset, uv.y), texture1, sampler0, offset);
    float West_height2 = getHeight(float2(uv.x - uv_offset, uv.y), texture1, sampler0, offset);
    //get the height of the texture at the current vertex
    float Current_height2 = getHeight(uv, texture0, sampler0,30.0f);
    
    //Cross multiplies the heights to get the appropirate height of the water mesh
    float North_height3 = cross(North_height, North_height2)/2;
    float South_height3 = cross(South_height, South_height2)/2;
    float East_height3 = cross(East_height, East_height2)/2;
    float West_height3 = cross(West_height, West_height2)/2;
    float Current_height3 = cross(Current_height, Current_height2)/2;
