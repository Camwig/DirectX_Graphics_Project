...
float4 main(InputType input) : SV_TARGET
{
    ...
    //Need to remeber to calculate the normals
    //input.normal = CalcNormal(input.tex, texture0, sampler0);
    

    //Texture2D new_texture = lerp(texture1.SampleLevel(sampler0, input.tex, 0).x, texture2.SampleLevel(sampler0, input.tex, 0), time);
    //input.normal = CalcNormal2(input.tex, texture2,texture1, sampler0,time);
    ...
    //    if (getHeight(input.tex) <= 10.0f)
    //{
    //    textureColour = lerp(texture0.Sample(sampler0, input.tex), texture1.Sample(sampler0, input.tex), 0.25);
    //}
