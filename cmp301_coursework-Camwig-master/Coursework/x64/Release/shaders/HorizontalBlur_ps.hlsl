...
float4 main(InputType input) : SV_TARGET
{
    ...
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 5.0f, 0.0f)) * (weight3);
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 4.0f, 0.0f)) * (weight2);
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 3.0f, 0.0f)) * (weight1);
